...
1. 自动识别FFAB文件格式和版本
2. 解析Meta信息区，获取图片数量、尺寸和压缩格式
3. 读取索引表，定位每张图片在文件中的位置
4. 使用astcenc工具解码ASTC压缩数据
5. 输出PNG格式的图片序列，文件名为frame_0000.png, frame_0001.png等

#### 注意事项

1. 需要安装astcenc工具并添加到系统PATH中
2. 输出文件夹会自动创建（如果不存在）
3. 解码过程中会显示进度信息
4. 所有图片将以RGBA格式保存，保持透明度信息
5. 如果FFAB文件格式无效或版本不支持，会显示错误信息

## ffab_info.py
这是FFAB文件格式的信息查看工具，用于分析FFAB文件的结构和内容，提供详细的统计信息。

### 使用方法

#### 基本语法
```bash
python ffab_info.py <input_file> [options]
```
...
1. 基本使用：
```bash
python ffab_info.py ./animation.ffab
```

2. 显示详细信息：
```bash
python ffab_info.py ./animation.ffab -v
```
...
```
============================================================
FFAB文件信息: ./animation.ffab
============================================================
文件大小: 1,234,567 字节 (1205.64 KB)
魔数: 0xFFAB ✓
版本: 0x0001 ✓
图片数量: 30
图片尺寸: 256x256
...
索引表位置: 偏移量 12
数据区位置: 偏移量 372
压缩数据总大小: 1,234,195 字节 (1205.27 KB)
压缩比: 6.35:1
每帧平均大小: 41139.83 字节
最小帧大小: 40960 字节
最大帧大小: 41472 字节
...
1. 如果FFAB文件格式无效或版本不支持，会显示错误信息
2. 使用-v参数可以查看每帧的详细索引信息
3. 工具会计算并显示压缩比和空间节省情况
4. 所有大小值同时以字节和KB/MB为单位显示
